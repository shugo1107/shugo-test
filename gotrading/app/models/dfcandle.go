...
func (df *DataFrameCandle) OptimizeEma(api *bitflyer.APIClient) (performance float64, bestPeriod1 int, bestPeriod2 int) {
	bestPeriod1 = 7
	bestPeriod2 = 14
	...
	for period1 := 5; period1 < 50; period1++ {
		for period2 := 12; period2 < 50; period2++ {
			...
			profit := signalEvents.Profit(api)
...
func (df *DataFrameCandle) OptimizeBb(api *bitflyer.APIClient) (performance float64, bestN int, bestK float64) {
	bestN = 20
	bestK = 2.0

	for n := 10; n < 60; n++ {
		for k := 1.0; k < 3.0; k += 0.1 {
...
func (df *DataFrameCandle) OptimizeIchimoku(api *bitflyer.APIClient) (performance float64) {
	signalEvents := df.BackTestIchimoku()
	if signalEvents == nil {
		return 0.0
	}
	performance = signalEvents.Profit(api)
	return performance
}
...
func (df *DataFrameCandle) OptimizeMacd(api *bitflyer.APIClient) (performance float64, bestMacdFastPeriod, bestMacdSlowPeriod, bestMacdSignalPeriod int) {
...
func (df *DataFrameCandle) OptimizeRsi(api *bitflyer.APIClient) (performance float64, bestPeriod int, bestBuyThread, bestSellThread float64) {
	bestPeriod = 14
...
func (df *DataFrameCandle) OptimizeParams() *TradeParams {
	emaPerformance, emaPeriod1, emaPeriod2 := df.OptimizeEma(bitflyer.New(config.Config.ApiKey, config.Config.ApiSecret))
	bbPerformance, bbN, bbK := df.OptimizeBb(bitflyer.New(config.Config.ApiKey, config.Config.ApiSecret))
	macdPerformance, macdFastPeriod, macdSlowPeriod, macdSignalPeriod := df.OptimizeMacd(bitflyer.New(config.Config.ApiKey, config.Config.ApiSecret))
	ichimokuPerforamcne := df.OptimizeIchimoku(bitflyer.New(config.Config.ApiKey, config.Config.ApiSecret))
	rsiPerformance, rsiPeriod, rsiBuyThread, rsiSellThread := df.OptimizeRsi(bitflyer.New(config.Config.ApiKey, config.Config.ApiSecret))
...
func (df *DataFrameCandle) ResetParams() *TradeParams {
